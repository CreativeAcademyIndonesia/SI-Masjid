...
const UpdateJadwalSholat = () => {
  ...
  useEffect(() => {
    ...
    if (typeof bulan == 'string') {
      setBulan(bulan);
      setTanggal(moment(tanggal).format('YYYY-MM-DD'));
    }
    ...
    <>
      ...
      <form onSubmit={submitHandler} className='container' style={{ maxWidth: '500px' }}>
        ...
        <div className="form-group">
          ...
          <input type="date" placeholder='Tanggal' className="form-control" id='tanggal' value={_tanggal} onChange={(e) => setTanggal(e.target.value)} />
        </div>
        <div className="form-group">
